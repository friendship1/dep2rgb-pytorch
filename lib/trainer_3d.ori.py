...
class SolidTrainer(BaseTrainer):
	...
	def project2d(self, F, coord2d):
		...
		# AssertionError : number of rows in features and coordinates do not match!
		...
		ret_dense_rsz = torch.nn.functional.interpolate(ret_dense_pad, size=[256,832], mode='nearest')
		# print("ret_rsz shape: ", ret_dense_pad.shape, ret_dense_rsz.shape)

		## DEBUG
		# print(ret_dense_pad.shape)
		# ret_np = ret_dense_pad.cpu().detach().numpy()
		# ret_np = ret_np.sum(axis=1)
		# print(ret_np.shape)
		# ret_np = ret_np != 0
		# # print(ret_np)
		# ret_np = ret_np.astype(np.uint8)
		# ret_np[0][0][0] = 1
		# ret_np = ret_np * 250
		# ret_np = np.vstack((ret_np,ret_np,ret_np))
		# print(ret_np.shape)

		# print(ret_np)
		# import cv2
		# ret_np = ret_np.astype(np.uint8)
		# ret_np = np.transpose(ret_np, (1,2,0))
		# print(ret_np.shape)
		# print(cv2.imwrite("debug.jpg", ret_np))
	...
	def _train_epoch(self, epoch):
		...
		for curr_iter in range(len(data_loader)):
			...
			feature_3d = self.model_3dnet(sparse_inp) # 32 dimensions
			feature_dense = self.project2d(feature_3d, input_dict['coord2d'])
			RGB = self.coarsenet(feature_dense)
			RGB = (RGB + 1) * 127.5
	...
	def _valid_epoch(self, is_test=False):
		...
		for batch_idx in range(tot_num_data):
			...
			# debug_feat = input_dict['feat']
			# debug_coord = input_dict['coord']
			# print("debug1", debug_coord.shape, debug_feat.shape)
			# np.savetxt("debug1.np.txt", debug_feat)
			# np.savetxt("debug1.np.txt", debug_coord)
